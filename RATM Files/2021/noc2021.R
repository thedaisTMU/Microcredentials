...
setwd("/Users/ibrahim/Desktop/Project/2021")
...
full.avg.crosswalk.skill <- full.crosswalk.skill[,mean(value),by=.(noc_2021,element.id,element.name,scale.id,scale.name)]
...
individual.ranking <- full.avg.crosswalk.skill[element.id %in% tech.skills,prod(V1),by=.(noc_2021,element.id)]
individual.ranking <- reshape(individual.ranking,direction="wide",v.names = c("V1"),timevar="element.id",idvar="noc_2021")
individual.ranking <- individual.ranking[!is.na(noc_2021)] #One O*NET occupation was not matched to NOC in the crosswalk and must be removed
setkey(individual.ranking,noc_2021)
...
individual.ranking$noc_code <- substr(individual.ranking$noc_2021, 1, 4)
individual.ranking$noc_name <- substr(individual.ranking$noc_2021, 5, nchar(individual.ranking$noc_2021))

#Write the CSV file containing the rankings
write.csv(individual.ranking,"tech.sector.def.2021.csv",row.names=FALSE)
...
kable(
  individual.ranking %>%
    ...
    select("Occupation" = noc_2021, "Tech Definition" = digital, "Tech Def w/o Telco" = tech.no.tel)
)
