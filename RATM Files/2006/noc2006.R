...
setwd("/Users/ibrahim/Desktop/Project/2006")
...
crosswalk <- as.data.table(read.csv("onetnoc_10_v1final.csv"))
...
full.avg.crosswalk.skill <- full.crosswalk.skill[,mean(value),by=.(noc_2006,element.id,element.name,scale.id,scale.name)]
...
individual.ranking <- full.avg.crosswalk.skill[element.id %in% tech.skills,prod(V1),by=.(noc_2006,element.id)]
individual.ranking <- reshape(individual.ranking,direction="wide",v.names = c("V1"),timevar="element.id",idvar="noc_2006")
individual.ranking <- individual.ranking[!is.na(noc_2006)] #One O*NET occupation was not matched to NOC in the crosswalk and must be removed
setkey(individual.ranking,noc_2006)
rows <- nrow(individual.ranking)


#------------------------------------------------------------------------
#IBRAHIM COMMENT -- Here, we basically filtered the full average crosswalk by 
#noc titles and multiplied the values of the importance to those surveyed (1-5), 
...
individual.ranking$noc_code <- substr(individual.ranking$noc_2006, 1, 4)
individual.ranking$noc_name <- substr(individual.ranking$noc_2006, 5, nchar(individual.ranking$noc_2006))

#Write the CSV file containing the rankings
write.csv(individual.ranking,"tech.sector.def.2006.csv",row.names=FALSE)
...
kable(
  individual.ranking %>%
    ...
    select("Occupation" = noc_2006, "Tech Definition" = digital, "Tech Def w/o Telco" = tech.no.tel)
)
